...
public class Publine_db_in_html_kousin1 extends LoggerFactory {
                ...
                if(pub_file[i].isFile()) {
                    try {
                        ...
                        while((strLine=br.readLine())!=null) {
                            ...
                            if(strLine.indexOf("var totalJson =")!=-1) {
                                strLine = strLine.trim();
                                strLine = strLine.replaceAll("var totalJson = ", "");
                                strLine = strLine.replaceAll(";", "");
                                
                                //strLine = strLine.replaceAll("{", "");
                                //strLine = strLine.replaceAll("}", "");
                                
                                String[] spl0 = strLine.split(",");
                                String tempo="",nyuko_s="",sale_t="",sale_s="",siire_s="";
                                String syuko_s="",henpin_s="",zaiko="",syoka_r="",henpin_r="";
                                for(int l=0;l<spl0.length;l++) {
                                    String[] spl1 = spl0[l].split(":");
                                    System.out.println(spl1[1]);
                                    spl1[1] = spl1[1].replaceAll("\"", "");
                                    spl1[1] = spl1[1].replaceAll("}", "");
                                    if(spl1[0].indexOf("purchaseTotal")>=-1) siire_s = spl1[1];
                                    if(spl1[0].indexOf("wareaTotal")>=-1) nyuko_s = spl1[1];
                                    if(spl1[0].indexOf("sales")>=-1) sale_t = spl1[1];
                                    if(spl1[0].indexOf("saleTotal")>=-1) sale_s = spl1[1];
                                    if(spl1[0].indexOf("issueTotal")>=-1) syuko_s = spl1[1];
                                    if(spl1[0].indexOf("returnSum")>=-1) henpin_s = spl1[1];
                                    if(spl1[0].indexOf("stock")>=-1) zaiko = spl1[1];
                                    if(spl1[0].indexOf("digestionRate")>=-1) syoka_r = spl1[1];
                                    if(spl1[0].indexOf("returnRate")>=-1) henpin_r = spl1[1];
                                }
                            }
                            if(strLine.indexOf("var data =")!=-1) {
