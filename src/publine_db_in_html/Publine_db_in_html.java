...
public class Publine_db_in_html extends LoggerFactory {
                ...
                if(pub_file[i].isFile()) {
                    try {
                        ...
                        while((strLine=br.readLine())!=null) {
                            if(strLine.indexOf("<TR bgColor=#e6e6fa>")!=-1) {
                                ...
                                while((strLine=br.readLine())!=null) {
                                    ...
                                    if(strLine.indexOf("<TH>")==-1) {
                                        //0:店舗、1:仕入累計、2:入庫累計、3:本日売上、4:売上累計、5:出庫累計
                                        //6:返品累計、7:在庫数量、8:消化率、9:返品率、10:棚卸日、
                                        //11:棚名称１、12:棚名称２、13:棚名称３
                                        
                                        strLine = strLine.replaceAll("<TD align=middle><B>", "");
                                        strLine = strLine.replaceAll("</B></TD>", "");
                                        strLine = strLine.replaceAll("<TD bgColor=#f8f8ff align=middle>", "");
                                        strLine = strLine.replaceAll("</TD>", "");                                    
                                        tempo = strLine.replaceAll(" ", "");//店舗
                                    }

                                    while((strLine=br.readLine()).indexOf("<TR>")==-1) {
                                        if(strLine.indexOf("<HR>")!=-1) {
                                            break;
                                        }
                                        else if(strLine.indexOf("<TH>")==-1) {
                                            strLine = strLine.replaceAll("<TD align=middle>", "");
                                            strLine = strLine.replaceAll("<B>", "");
                                            strLine = strLine.replaceAll("</B>", "");
                                            strLine = strLine.replaceAll("<TD>", "");
                                            strLine = strLine.replaceAll("</TD>", "");
                                            strLine = strLine.replaceAll("<TD align=right>", "");
                                            strLine = strLine.replaceAll("<TD align=left>", "");
                                            strLine = strLine.replaceAll("</TR>", "");
                                            strLine = strLine.replaceAll("<FONT color=#009900>", "");
                                            strLine = strLine.replaceAll("<FONT color=#cc0033>", "");
                                            strLine = strLine.replaceAll("</FONT>", "");
                                            strLine = strLine.replaceAll("</TBODY></TABLE><BR>", "");
                                            
                                            if(num==0) siire_s = strLine.replaceAll("　", "");//仕入累計
                                            if(num==1) nyuko_s = strLine.replaceAll("　", "");//入庫累計
                                            if(num==2) sale_t = strLine.replaceAll("　", "");//本日売上
                                            if(num==3) sale_s = strLine.replaceAll("　", "");//売上累計
                                            if(num==4) syuko_s = strLine.replaceAll("　", "");//出庫累計
                                            if(num==5) henpin_s = strLine.replaceAll("　", "");//返品累計
                                            if(num==6) zaiko = strLine.replaceAll("　", "");//在庫数量
                                            if(num==7) syoka_r = strLine.replaceAll("　", "");//消化率
                                            if(num==8) henpin_r = strLine.replaceAll("　", "");//返品率
